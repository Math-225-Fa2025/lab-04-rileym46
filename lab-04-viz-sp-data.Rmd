...
However note that the data scraping was also done in R, and we will discuss web scraping using R later in the course.
...
`1643 rows and 6 columns. Each row represents a location. The varaibles are address, city, state, zip code, longitude and latitude.`

2.  What are the dimensions of the La Quinta's dataset?
    What does each row in the dataset represent?
    What are the variables?
  
`909 rows and 6 columns. The each row represents a La Quinta location. The variables are address, city, state, zip code, latitude, and longitude.`
...
3.  Take a look at the websites that the data come from (linked above).
    ...
    What about Denny's?
    `Yes for La Quinta (Canada, Mexico, China, New Zealand, Georgia, Turkiye, United Arab Emirates, Colombia, and Ecuador) and Denny's (Mexico).`
...
4.  Now take a look at the data.
    ...
    `Filtering it out would be an option, but it would be difficult because there isn't a country locator, just states. Another option would be to use use an anti_join to see if there are any x variables that don't have a y value (state abbreviation).`
...
7.  Find the La Quinta locations that are outside the US, and figure out which country they are in.
    ...
    `Canada, Mexico, China, New Zealand, Georgia, Turkiye, United Arab Emirates, Colombia, and Ecuador.`
...
```{r laquinta-usa, eval = TRUE}
laquinta <- laquinta %>%
  filter(state %in% states$abbreviation)
```

9.  Which states have the most and fewest Denny's locations? What about La Quinta? Is this surprising? Why or why not? `California has the most Denny's location, and Deleware has the fewest. Texas has the most La Quinta's, and Maine only has 1. This makes sense because of the relative populations and size of the states.`

```{r location-frequency, eval = TRUE}
dennys %>%
  count(state) %>%
  arrange(desc(n))
```

```{r location-frequency-min, eval = TRUE}
dennys %>%
  count(state) %>%
  arrange(n)
```

```{r location-frequency-lq, eval = TRUE}
laquinta %>%
  count(state) %>%
  arrange(desc(n))
```
...
```{r location-frequency-lq-min, eval = TRUE}
laquinta %>%
  count(state) %>%
  arrange(n)
```
...
`The joke does seem to hold. For the La Quinta locations, they are fairly close to a Denny's. Not all Denny's are close to a La Quinta, though.`

12. Now filter the data for observations in Texas only, and recreate the plot, with an appropriate `alpha` level.
    Visually, does Mitch Hedberg's joke appear to hold here?
    
```{r graph-TX, eval = TRUE}
dn_lq %>%
  filter(state == "TX") %>%
  ggplot(dn_lq, mapping = aes(x = longitude, y = latitude, color = establishment)) +
  geom_point(alpha= 0.5) 
...
`There are a LOT more La Quintas than Denny's here, located all on top of each other. They are definitely next to each other.`
